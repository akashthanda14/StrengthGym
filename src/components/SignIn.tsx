...
const SignIn: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('https://strengthgymbackend.onrender.com/api/auth/login', {
        method: 'POST',
    ...
    >
      ...
      <div className="w-full max-w-md z-10">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">STRENGTH GYM PHILLAUR</h1>
          <p className="text-[#a0a0a0]">Push your limits. Track your progress.</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm">
                <a href="#" className="font-medium text-[#8b0000] hover:text-[#ff6b6b]">
                  Forgot password?
                </a>
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-3 px-4 rounded-lg font-medium text-white bg-[#8b0000] hover:bg-[#a52a2a] focus:outline-none focus:ring-2 focus:ring-[#8b0000] focus:ring-offset-2 focus:ring-offset-[#2A2D33] transition ${
